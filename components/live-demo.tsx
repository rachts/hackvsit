...
import { Scan, ShieldAlert, ShieldCheck, CheckCircle2, AlertCircle, Loader2, UploadCloud, X } from "lucide-react"
...
export function LiveDemo() {
  ...
  const processFile = async (selectedFile: File) => {
    ...
    try {
      ...
      console.error(error)
      setErrorMessage(error.message || "Failed to process image. Make sure the OCR backend is running.")
      setStatus("error")
    }
  }
...
function ResultItem({ label, value, icon, loading, valueColor = "text-on-surface" }: { label: string, value: string, icon: string, loading?: boolean, valueColor?: string }) {
  return (
